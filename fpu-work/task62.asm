...
;       0000000000000001 ; Выполнена недопустимая операция
;       0000000000000010 ; Выполнена операция над денормализованным числом
;       0000000000000100 ; Деление на ноль
;       0000000000001000 ; Результат слишком большой
;       0000000000010000 ; Результат слишком маленький
;       0000000000100000 ; Результат не может быть представлен
; Проверка ошибок вычисления
checksr PROC
	push ax
	fstsw ax ; считываем содержимое регистра sr
	; В зависимости от установленных битов - проверяем ошибки
	...
	pop ax
	ret
	; Выводим сообщения об ошибке в зависимости от установленного бита
	_wrongop:
...
calcexpr PROC STDCALL
	LOCAL val:WORD
	; Кладем на стек числа
	fld ds:numbera
	fld ds:numberb
	fld ds:numberc ; c - b - a
;	int 3h
	fmul st(0), st(2) ; a*c - b - a
	call checksr		; проверяем операцию
...
start:
	...
	fstcw cr			; считываем CR в cr
	or cr,  0000000011000000b	; точность вычислений - 80 бит
	fldcw cr			; загружаем CR из cr
	; A
	mov dx, offset entera	
	mov ah, 09h
	int 21h
	push 30 ; строку такого размера можем считать (см. на размер буфера)
	push offset string ; передаём адрес начала буфера
	call readstring
	push offset string
	call strtofloat
	fstp numbera		; считываем со стека число
	call nextline
	; B
	...
	push offset string
	call strtofloat
	fstp numberb		; считываем со стека число
	call nextline
	; C
	...
	push offset string
	call strtofloat
	fstp numberc		; считываем со стека число
	call nextline
;	int 3h
	call calcexpr		; вычисляем выражение
